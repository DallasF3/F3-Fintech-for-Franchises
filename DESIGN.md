...
**`footfall-data-card`** — A standard analytics dashboard component for displaying Purple WiFi footfall and dwell time. Matches the `property-card` bounding box: white surface, `{rounded.md}` clipping, 24px padding, with a `{typography.display-md}` primary stat (e.g., "14 mins" or "320 visitors") and `{typography.body-sm}` trend microcopy.
**`integration-status-badge`** — A pill-shaped indicator (`{rounded.full}`) for integration health (e.g., Purple WiFi sync status). Green/Red indicator dots alongside text.
